...
public sealed partial class MainViewModel : ObservableObject, IDisposable
{
    ...
    public void AddPresetWidget(string presetKey)
    {
        ...
        (int w, int h, WidgetType type, string key, string template, string name, bool font, bool border, bool marquee) config = presetKey?.ToLowerInvariant() switch
        {
            ...
            "cpu_clock_mhz" or "cpu_clock" => (48, 10, WidgetType.Text, "cpu_clock", "{cpu_clock}MHz", $"CPU MHz #{count}", false, false, false),
            "cpu_clock_ghz" => (48, 10, WidgetType.Text, "cpu_clock", "{cpu_clock_ghz}GHz", $"CPU GHz #{count}", false, false, false),
            "gpu_clock"     => (48, 10, WidgetType.Text, "gpu_clock", "{gpu_clock}MHz", $"GPU MHz #{count}", false, false, false),
            "gpu_mem_clock" => (52, 10, WidgetType.Text, "gpu_mem_clock", "{gpu_mem_clock}MHz", $"VRAM MHz #{count}", false, false, false),
            "ram_clock"     => (52, 10, WidgetType.Text, "ram_clock", "{ram_clock}MT/s", $"RAM MT/s #{count}", false, false, false),
